...
function applyPlayNextFilters(games, options = {}) {
...
function buildRecommendationReasons(game, context = {}) {
  const reasons = [];
...
function buildPlayNextRecommendations(library, options = {}) {
  ...
  const scored = eligibleGames.map((game) => {
    ...
    return {
      ...game,
      score,
      reasons: buildRecommendationReasons(game, { score })
    };
  });
...
function createRecommendationState(initial = {}) {
  return {
...
module.exports = {
  buildPlayNextRecommendations,
  applyPlayNextFilters,
