...
assert.ok(app.includes('width >= 900 && height >= 400'), 'low-resolution artwork should be rejected');
...
assert.ok(css.includes('backdrop-filter: blur(8px)'), 'background should include a dark blur overlay');
...
assert.ok(sw.includes('project-sora-steam-discovery-v13'), 'service worker cache should be bumped');
